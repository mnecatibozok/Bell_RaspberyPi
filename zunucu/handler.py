...
# Kamera modülü (Raspberry Pi'de aktif, diğerlerinde graceful)
try:
    import kamera as _kamera
    _kamera.baslat()
    _KAM_OK = True
except Exception as _kam_hata:
    print(f'[KAM] Kamera modülü yüklenemedi: {_kam_hata}')
    _KAM_OK = False
...
try:
    ...
    _RELAY_OK = True
except Exception as _relay_hata:
    print(f'[RELAY] Relay modülü yüklenemedi: {_relay_hata}')
    _RELAY_OK = False
...
class ZilHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/video_feed':
            return self._get_video_feed()

        if path == '/api/kamera-durum':
            return self._get_kamera_durum()

        if path == '/api/kamera-log':
            return self._get_kamera_log()

        if path == '/api/kamera-snapshots':
            return self._get_kamera_snapshots()

        if path == '/api/relay-durum':
            return self._get_relay_durum()

        if path.startswith('/img/'):
            return self._serve_img(path)
    ...
    def do_POST(self):
        ...
        if path == '/api/kamera-esik':
            return self._post_kamera_esik(body)

        if path == '/api/kamera-mod':
            return self._post_kamera_mod(body)

        if path == '/api/relay-ac':
            if _RELAY_OK: _relay.ac()
            return self._serve_json({'ok': True, 'durum': _relay.get_durum() if _RELAY_OK else {}})

        if path == '/api/relay-kapat':
            if _RELAY_OK: _relay.kapat()
            return self._serve_json({'ok': True, 'durum': _relay.get_durum() if _RELAY_OK else {}})

        if path == '/api/kamera-log-temizle':
            return self._post_kamera_log_temizle()
    ...
    def _get_kamera_durum(self):
        if not _KAM_OK:
            self._serve_json({'bagli': False, 'hareket_var': False,
                              'bugun_sayisi': 0, 'son_tespit': '—',
                              'ders_saati': False, 'esik': 50})
            return
        qs   = urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query)
        ders = qs.get('ders', ['0'])[0] == '1'
        self._serve_json(_kamera.get_durum(ders_saati_aktif=ders))

    def _get_kamera_log(self):
        if not _KAM_OK:
            self._serve_json({'kayitlar': [], 'esik': 50,
                              'bugun_sayisi': 0, 'ders_sayisi': 0})
            return
        self._serve_json(_kamera.get_log())

    def _post_kamera_esik(self, body: dict):
        if _KAM_OK:
            _kamera.set_esik(body.get('esik', 50))
        self._serve_json({'ok': True})

    def _post_kamera_mod(self, body: dict):
        if _KAM_OK:
            _kamera.set_mod(body.get('her_zaman', False))
        self._serve_json({'ok': True})

    def _post_kamera_log_temizle(self):
        if _KAM_OK:
            _kamera.temizle_log()
        self._serve_json({'ok': True})

    def _get_relay_durum(self):
        d = _relay.get_durum() if _RELAY_OK else {'anfi_acik': False, 'gpio_ok': False, 'pin': 17}
        self._serve_json(d)

    def _get_kamera_snapshots(self):
        from pathlib import Path
        img_dizin = Path(__file__).parent.parent / 'img'
        gunler = []
        if img_dizin.exists():
            for gun in sorted(img_dizin.iterdir(), reverse=True):
                if gun.is_dir():
                    resimler = sorted([f.name for f in gun.iterdir() if f.suffix.lower() == '.jpg'], reverse=True)
                    if resimler:
                        gunler.append({'gun': gun.name, 'resimler': resimler, 'sayi': len(resimler)})
        self._serve_json({'gunler': gunler})

    def _serve_img(self, path: str):
        from pathlib import Path
        # path: /img/20260525/123456_78.jpg
        rel = path.lstrip('/')
        dosya = Path(__file__).parent.parent / rel
        if not dosya.exists() or dosya.suffix.lower() not in ('.jpg', '.jpeg', '.png'):
            self.send_response(404); self.end_headers(); return
        data = dosya.read_bytes()
        self.send_response(200)
